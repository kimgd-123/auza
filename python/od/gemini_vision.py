...
# 텍스트 영역용 프롬프트
PROMPT_TEXT = """이 영역의 콘텐츠를 분석하여 HTML로 구조화해주세요.
...
# 표 영역용 프롬프트
PROMPT_TABLE = """이 영역의 표를 HTML <table>로 구조화해주세요.
...
⚠️ HTML만 반환하고 마크다운 코드블록(```)이나 <html>/<body> 태그로 감싸지 마세요"""

# 수식 영역용 프롬프트
PROMPT_FORMULA = """이 영역의 수식을 LaTeX 형식으로 변환해주세요.
