"""Python 측 Gemini Vision 클라이언트 — OD 크롭 영역별 호출"""

import sys
import base64


# ── 수식 공통 규칙 (모든 프롬프트에서 재사용) ──
_MATH_RULES = """
## ⚠️ 수식 처리 — 최우선 규칙! ⚠️
모든 수학 수식은 **반드시** 달러 기호로 감싸서 LaTeX 형식으로 출력하세요.

**필수 규칙:**
- 인라인 수식: `$수식$` (예: "함수 $f(x) = x^2$의 값")
- 블록 수식: `$$수식$$` (예: $$\\frac{a}{b}$$)
- 수식 내 부등호: < → &lt;, > → &gt; (HTML 엔티티로 변환!)

**변환 예시:**
- 분수 a/b → $\\frac{a}{b}$
- 제곱근 √x → $\\sqrt{x}$
- 지수 x² → $x^2$, 아래첨자 x₁ → $x_1$
- 그리스 문자: α→$\\alpha$, β→$\\beta$, π→$\\pi$
- 특수기호: ∞→$\\infty$, ≤→$\\leq$, ≥→$\\geq$, ≠→$\\neq$

⚠️ **위첨자/아래첨자 감지 — 절대 놓치지 마세요!** ⚠️
이미지에서 글자 크기가 작거나 위/아래로 치우친 숫자/문자는 반드시 지수(^) 또는 아래첨자(_)로 변환하세요!
- x2 → $x^2$ (2가 위에 작게 있으면 지수)
- P(x)=4x4 → $P(x)=4x^4$ (4가 위첨자)
- (n-2)2 → $(n-2)^2$ (2가 위첨자)
- a1, a2 → $a_1$, $a_2$ (숫자가 아래에 작게 있으면 아래첨자)

⚠️ 변수 바로 뒤에 오는 숫자가 **본문 크기보다 작거나 위/아래로 치우쳐 있으면** 반드시 ^(지수) 또는 _(아래첨자)를 사용하세요!

⚠️ 수식이 있으면 반드시 $로 감싸세요! √, ², ₁ 같은 유니코드를 그대로 두지 말고 반드시 LaTeX로 변환하세요!"""

_NO_HALLUCINATION = """
## ⚠️ 절대 금지 사항 ⚠️
- 이미지에 없는 내용을 **절대 추가하지 마세요** (해설, 풀이, 답 생성 금지)
- 보이는 텍스트만 정확히 변환하세요
- 추론이나 계산을 하지 마세요
- HTML만 반환하고 마크다운 코드블록(```)이나 <html>/<body> 태그로 감싸지 마세요"""


# 텍스트 영역용 프롬프트
PROMPT_TEXT = f"""이 영역의 콘텐츠를 분석하여 HTML로 구조화해주세요.

## 출력 규칙
- 일반 텍스트 → HTML (<p>, <b>, <i> 등)
- 표 → HTML <table> (셀 병합 colspan/rowspan, 스타일 포함). <thead>/<tbody> 사용 금지, <tr>+<th>/<td> 직접 사용
- 빈칸/답란 (□, 네모 상자) → 빈 괄호 `( )` 또는 밑줄 `____`로 표현

## ⚠️ 선지(답 보기) — 테이블 금지! ⚠️
①②③④⑤ 또는 ⑴⑵⑶ 번호가 붙은 선지(답 보기)는 **절대 <table>로 변환하지 마세요.**
- 원본처럼 한 줄에 여러 선지가 나열되면 → `<p>① ㄱ &nbsp;&nbsp; ② ㄱ, ㄴ &nbsp;&nbsp; ③ ㄱ, ㄷ &nbsp;&nbsp; ④ ㄴ, ㄷ &nbsp;&nbsp; ⑤ ㄱ, ㄴ, ㄷ</p>` 형태로 출력
- 원본에서 2줄로 나뉘어 있으면 각각 `<p>`로 출력
- 선지를 표(table)로 만들면 HWP 변환 시 레이아웃이 깨지므로 반드시 `<p>` 태그로 출력하세요!

## ⚠️ 박스/테두리 감지 — 반드시 감지하세요! ⚠️
텍스트 주위에 사각형 테두리(선)가 있는 모든 경우를 감지하여 `<div style="border: ...">`로 감싸세요.

**반드시 감지해야 하는 박스 유형:**
- **지문 박스**: 긴 텍스트가 테두리로 둘러싸인 경우
- **보기 박스**: <보기>, ㄱ, ㄴ, ㄷ 등이 테두리 안에 있는 경우
- **인용문 박스**: 인용문이나 참고 자료가 테두리로 구분된 경우
- **정의/공식 박스**: 수학 정의, 공식, 정리가 테두리 안에 있는 경우
- **회색/음영 배경**: 배경색이 있는 영역 → `background-color` 추가

**HTML 변환 규칙:**
- 실선 테두리 → `<div style="border: 1px solid #000; padding: 8px;">`
- 둥근 테두리 → `border-radius: 4px` 추가
- 배경색 → `background-color: #f0f0f0` 등 추가

⚠️ 테두리가 보이면 절대 무시하지 마세요! 반드시 `<div style="border:...">`로 감싸세요!

## ⚠️ 그림/이미지 감지 — 반드시 지켜주세요! ⚠️
그래프, 차트, 다이어그램, 삽화, 사진, 도형, 지도 등 **시각 자료**가 보이면:
- 해당 위치에 `[FIGURE]`를 출력하세요
- 시각 자료 주변의 텍스트는 정상적으로 HTML로 변환하세요
- 예: 텍스트 → `<p>발문 텍스트</p>\n[FIGURE]\n<p>보기 텍스트</p>`

{_MATH_RULES}
{_NO_HALLUCINATION}"""

# 표 영역용 프롬프트
PROMPT_TABLE = f"""이 영역의 표를 HTML <table>로 구조화해주세요.

## 출력 규칙
- HTML <table> 사용 (셀 병합 colspan/rowspan, 스타일 포함)
- <thead>/<tbody> 사용 금지, <tr>+<th>/<td> 직접 사용
- 배경색이 있으면 style="background-color: #xxx" 추가
- 셀 내 수식은 $...$로 감싸기
{_MATH_RULES}
{_NO_HALLUCINATION}"""

# figure 재분류 프롬프트 — 텍스트/수식이면 파싱, 진짜 이미지면 [FIGURE] 반환
PROMPT_FIGURE_CHECK = f"""이 영역을 분석해주세요.

## 판별 기준
- **텍스트/수식 영역**: 글자, 숫자, 수학 기호, 선지(①②③④⑤) 등이 주된 내용이면 → HTML로 파싱하세요
- **그림/도표/사진 영역**: 그래프, 차트, 다이어그램, 삽화, 사진, 도형 등 시각 자료이면 → `[FIGURE]` 한 단어만 반환하세요

## 텍스트/수식인 경우 출력 규칙
- 일반 텍스트 → HTML (<p>, <b>, <i> 등)
- 표 → HTML <table>
- 빈칸/답란 → 빈 괄호 `( )` 또는 밑줄 `____`
- ①②③④⑤ 선지(답 보기)는 절대 <table>로 만들지 말고 <p>로 출력

{_MATH_RULES}
{_NO_HALLUCINATION}"""

# 수식 영역용 프롬프트
PROMPT_FORMULA = f"""이 영역의 수식을 LaTeX 형식으로 변환해주세요.

## 출력 규칙
- 블록 수식: $$수식$$ 형식으로 출력
- 분수, 루트, 첨자, 합, 적분, 극한 등 모든 수식 구조를 LaTeX로 정확히 변환
- 수식 전후 텍스트가 있으면 <p> 안에 포함
{_MATH_RULES}
{_NO_HALLUCINATION}"""


# ── 정답·풀이 추론용 프롬프트 (v2.5.0) ──
PROMPT_ANSWER_SOLUTION = """이 이미지는 수학 문제(또는 여러 문제) 가 포함된 영역입니다.
각 문제의 정답과 풀이를 추론하여 JSON 으로 반환하세요.

## 출력 스키마 (JSON)
{
  "items": [
    {
      "questionNo": "1",            // 문항 번호 (보이는 그대로, 없으면 "")
      "answer": "③",                // 정답 — 객관식이면 ①②③④⑤ 중 하나, 주관식이면 답 (수, 식, 텍스트)
      "solution": "f(x)=...에서..." // 풀이 — 핵심 과정만 간결하게 (수식은 LaTeX $...$)
    }
  ]
}

## 규칙
- 이미지에 보이는 문제만 다루세요. 보이지 않는 추가 문제를 만들지 마세요.
- 문항 번호가 보이지 않으면 questionNo 는 빈 문자열("")로 두세요.
- 객관식 선지가 있으면 정답을 ①②③④⑤ 중 하나로 표기하세요.
- 주관식/서술형은 자연어 또는 수식 답으로 작성하세요.
- 풀이는 핵심 단계만 간결하게 (3~6줄 권장). 수식은 LaTeX 인라인 `$...$` 형식.
- 풀이 안에서 부등호는 그대로 사용 (HTML 엔티티 변환 불필요).
- 정답·풀이를 확신할 수 없으면 해당 필드를 빈 문자열("")로 두세요. 추측해서 채우지 마세요.
- JSON 외 다른 텍스트(마크다운 코드블록, 설명)는 절대 출력하지 마세요."""


# 정답·풀이 응답 JSON 스키마 (google-genai SDK)
ANSWER_SOLUTION_SCHEMA = {
    "type": "object",
    "properties": {
        "items": {
            "type": "array",
            "items": {
                "type": "object",
                "properties": {
                    "questionNo": {"type": "string"},
                    "answer": {"type": "string"},
                    "solution": {"type": "string"},
                },
                "required": ["answer", "solution"],
            },
        },
    },
    "required": ["items"],
}


# thinking_budget 기본값 (v2.5.0): 수학 추론용 적당한 토큰 한도
# -1 = 자동 (SDK 가 결정), 0 = 비활성, 양수 = 명시 한도
_DEFAULT_ANSWER_THINKING_BUDGET = -1


def _strip_code_fences(text: str) -> str:
    """Gemini 응답에서 코드 펜스 및 HTML 래퍼 제거"""
    import re

    text = text.strip()

    # 코드 펜스 제거
    if text.startswith("```html"):
        text = text[7:]
    elif text.startswith("```"):
        text = text[3:]
    if text.endswith("```"):
        text = text[:-3]
    text = text.strip()

    # <html>/<body> 래퍼 제거
    text = re.sub(r'</?(!doctype[^>]*|html|head|body)[^>]*>', '', text, flags=re.IGNORECASE)

    return text.strip()


# ── VisionClient 싱글턴 (api_key별 캐싱은 GeminiDirectClient 내부에서 처리) ──
_vision_clients: dict = {}  # {api_key: GeminiDirectClient}


def _get_vision_client(api_key: str):
    """api_key별 GeminiDirectClient 싱글턴 반환"""
    from .vision_client import GeminiDirectClient

    if api_key not in _vision_clients:
        _vision_clients[api_key] = GeminiDirectClient(api_key)
    return _vision_clients[api_key]


def call_gemini_vision(api_key: str, image_base64: str, prompt: str) -> str:
    """Gemini Vision API 호출

    Args:
        api_key: Gemini API 키
        image_base64: base64 인코딩된 PNG 이미지
        prompt: 분석 프롬프트

    Returns:
        Gemini 응답 텍스트 (HTML)
    """
    client = _get_vision_client(api_key)
    result = client.call_vision(image_base64, prompt)
    return _strip_code_fences(result.text)


def call_gemini_answer_solution(
    api_key: str,
    image_base64: str,
    thinking_budget: int = _DEFAULT_ANSWER_THINKING_BUDGET,
    timeout: float = 90,
) -> dict:
    """정답·풀이 추론용 Gemini Vision JSON 호출 (v2.5.0)

    Args:
        api_key: Gemini API 키
        image_base64: 세그먼트(블록) 전체 이미지 base64
        thinking_budget: thinking 토큰 한도 (-1=자동, 0=비활성)
        timeout: 요청 타임아웃 (초)

    Returns:
        {"items": [{"questionNo", "answer", "solution"}, ...]}
        - 응답 파싱 실패 시 {"items": []} 반환 (호출자가 빈 답으로 처리)
    """
    client = _get_vision_client(api_key)
    result = client.call_vision(
        image_base64,
        PROMPT_ANSWER_SOLUTION,
        timeout=timeout,
        json_mode=True,
        response_schema=ANSWER_SOLUTION_SCHEMA,
        thinking_budget=thinking_budget,
    )
    raw = (result.text or "").strip()
    if not raw:
        return {"items": []}

    # JSON 응답이라도 SDK 가 가끔 코드펜스를 붙이는 경우 대비
    raw = _strip_code_fences(raw)

    import json as _json
    try:
        parsed = _json.loads(raw)
    except _json.JSONDecodeError:
        sys.stderr.write(f"[gemini-vision] answer/solution JSON 파싱 실패: {raw[:200]}\n")
        return {"items": []}

    items = parsed.get("items") if isinstance(parsed, dict) else None
    if not isinstance(items, list):
        return {"items": []}

    # 각 item 검증/정규화
    normalized = []
    for it in items:
        if not isinstance(it, dict):
            continue
        normalized.append({
            "questionNo": str(it.get("questionNo") or "").strip(),
            "answer": str(it.get("answer") or "").strip(),
            "solution": str(it.get("solution") or "").strip(),
        })
    return {"items": normalized}


def get_prompt_for_region(region: str) -> str:
    """영역 타입에 맞는 프롬프트 반환"""
    if region == "table":
        return PROMPT_TABLE
    elif region == "formula":
        return PROMPT_FORMULA
    elif region == "boxed_text":
        return PROMPT_TEXT  # 테두리는 코드가 후처리로 래핑하므로 텍스트 프롬프트 사용
    else:
        return PROMPT_TEXT
