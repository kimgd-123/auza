...
### 1.2 한 줄 요약
PDF에서 텍스트를 복사하고, 표/수식은 Gemini Vision으로 구조화하여 재구성한 뒤, 편집 결과를 COM 인라인 자동화로 HWP·PPT에 자동 작성하는 Windows 데스크톱 앱
...
### 1.4 핵심 가치
1. **PDF → 에디터 → HWP/PPT** 워크플로우를 하나의 앱에서 완결
2. **Gemini Vision**으로 표/수식/복잡 콘텐츠를 AI가 구조화하여 재구성
3. **Gemini AI 채팅**으로 텍스트/표를 즉시 수정·교정 가능
4. **COM 인라인 자동화**로 표 스타일·수식까지 재현하여 HWP·PPT 자동 작성

### 1.5 제품 로드맵

| 단계 | 범위 | 설명 |
|------|------|------|
| **v1.0** | HWP 작성 | 한글 오토메이션 API (COM/OLE) 기반 HWP 자동 작성 |
| **v2.0** | PPT 작성 확장 | PowerPoint COM 자동화 추가, HWP/PPT 선택 작성 |
...
| Phase | 내용 | 목표 |
|-------|------|------|
| **Phase 9** | PPT COM 자동화 엔진 | ppt_writer.py, 텍스트/표/스타일 PPT 작성 |
| **Phase 10** | UI 확장 — 출력 대상 선택 | HWP/PPT 토글, PPT 슬라이드 레이아웃 설정 |
| **Phase 11** | PPT 통합 테스트 + 릴리즈 | PPT 전체 플로우 테스트, exe 재패키징 |
...
### 13.1 PPT COM 자동화 방식
- **win32com COM 인라인 방식** 채택 (HWP 오토메이션과 동일한 UX)
- PowerPoint 프로그램이 열리며 실시간으로 슬라이드/표/텍스트 작성
- python-pptx(파일 생성 방식) 대비 스타일 제어 완전 지원

### 13.2 PPT COM 주요 객체

| COM 객체 | 용도 |
|----------|------|
| `win32com.client.Dispatch("PowerPoint.Application")` | PPT 앱 실행 |
| `Presentations.Add()` | 새 프레젠테이션 |
| `Slides.Add(Index, Layout)` | 슬라이드 추가 |
| `Shapes.AddTable(Rows, Cols, L, T, W, H)` | 표 생성 |
| `Shapes.AddTextbox(Orient, L, T, W, H)` | 텍스트 상자 |
| `Table.Cell(row, col).Merge(Cell2)` | 셀 병합 |
| `Cell.Shape.Fill.ForeColor.RGB` | 셀 배경색 |
| `Cell.Borders(index).Weight / .Color` | 테두리 설정 |
| `TextRange.Font.Bold / .Italic / .Size / .Name` | 텍스트 서식 |
| `Presentation.SaveAs(path)` | 저장 |

### 13.3 HTML → PPT 변환 매핑

| HTML 요소 | PPT 변환 |
|-----------|----------|
| `<p>`, `<div>` | TextBox 또는 슬라이드 내 텍스트 |
| `<b>`, `<strong>` | Font.Bold |
| `<i>`, `<em>` | Font.Italic |
| `<u>` | Font.Underline |
| `<table>` | Shapes.AddTable |
| `colspan/rowspan` | Cell.Merge |
| `border` | Cell.Borders |
| `background-color` | Cell.Shape.Fill |
| `text-align` | ParagraphFormat.Alignment |
| `font-size` | Font.Size |

### 13.4 UI 변경 사항 (v2.0)
- 에디터 블록 상단에 **출력 대상 토글**: `[HWP] [PPT]`
- PPT 선택 시 추가 옵션: 슬라이드 레이아웃 (빈 슬라이드 / 제목+내용)
- "전체 작성" 버튼이 선택된 출력 대상에 따라 HWP 또는 PPT 실행
- 에디터 블록 1개 = 슬라이드 1장 매핑 (기본값, 설정 변경 가능)

### 13.5 아키텍처 설계 원칙
- `hwp_writer.py`와 `ppt_writer.py`는 동일한 인터페이스(추상 클래스) 구현
- `html_parser.py`는 출력 대상에 무관하게 공통 중간 구조로 변환
- 출력 대상별 Writer만 교체하는 Strategy 패턴 적용

```
HTML (TipTap)
   ↓ html_parser.py
DocumentStructure (공통 중간 구조)
   ↓ (Strategy)
   ├── hwp_writer.py → HWP
   └── ppt_writer.py → PPT
```
