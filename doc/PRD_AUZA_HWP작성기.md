...
> **버전**: v2.1
> **작성일**: 2026-03-24
> **상태**: Draft (v2.1 OD Review Step 추가)
...
| **v2.1** | 2.1.0 ~ | OD 인식 품질 안정화 | OD 검출 결과 사용자 리뷰/편집 단계 추가, IPC 분리(detect+convert) |
...
#### 4.3.5 OD Review Step (v2.1)

OD ON 캡처 시 YOLO 검출과 Gemini 변환 사이에 **사용자가 검출 결과를 확인/편집할 수 있는 선택적 단계**를 추가한다.

**기존 흐름 (OD Review OFF, 기본값)**:
영역 드래그 → YOLO 검출 + Gemini 변환 (atomic) → 에디터 삽입

**v2.1 흐름 (OD Review ON)**:
영역 드래그 → YOLO 검출 → **검출 결과 오버레이 표시 + 사용자 편집** → Gemini 변환 + figure 후처리 → 에디터 삽입

| 기능 | 설명 |
|------|------|
| **검출 결과 오버레이** | 캡처 이미지 위에 YOLO 바운딩 박스를 색상별로 표시 (text=blue, table=green, figure=purple, formula=orange, abandon=red) |
| **바운딩 박스 편집** | 이동, 8방향 리사이즈로 영역 범위 조정 |
| **유형 변경** | 드롭다운으로 영역 유형 변경 (text, table, figure, formula, abandon) |
| **영역 삭제** | 불필요한 검출 영역 제거 (Delete 키 / 버튼) |
| **영역 수동 추가** | 드래그로 새 바운딩 박스 생성, 유형 지정 |
| **OD Review 토글** | OD ON일 때만 표시, 기본 OFF (기존 동작 유지) |
| **Navigation Lock** | 리뷰 중 페이지 이동/줌 변경/툴 전환 시 discard 확인 |
| **재시도** | 변환 실패 시 편집된 detections 기반 convert 재실행 (detect 재실행 아님) |

> **설계 원칙**: detect 시점에 `PendingOdReview` 세션 객체를 생성하여 source context(pdfPath, pageNum, captureBboxNorm, blockId, imageBase64)를 immutable하게 고정. convert 직전 reading-order 재정렬 적용.

> **IPC 분리**: 기존 `capture:analyze` (atomic) 유지 + `capture:detect` (OD only) + `capture:convert` (Gemini + figure post-processing) 추가

#### 4.3.6 캡처 후 사용자 워크플로우
1. 캡처 → Gemini Vision 인식 (또는 OD Review를 거쳐 변환) → 에디터에 자동 삽입
...
| 기능 | 설명 |
|------|------|
| 블록 연동 | 선택된 에디터 블록의 내용을 컨텍스트로 전달 — 체크박스로 다중 선택된 블록이 있으면 그 전체, 없으면 활성 포커스 블록 단일 (v2.3.3~) |
...
| **Phase 12** | OD Review Step | YOLO 검출 결과 사용자 리뷰/편집 UI, IPC 분리(detect+convert), 테스트 인프라 |
...
### 13.8 Phase 12 — OD Review Step (v2.1)

#### 배경
OD ON 캡처 시 Gemini Vision의 서버측 모델 변동, YOLO 오검출 등으로 인식 품질이 불안정하여 현업 사용이 어려움. YOLO 검출 결과를 사용자가 검증/수정한 뒤 Gemini 변환을 실행하는 선택적 리뷰 단계를 추가하여 인식 안정성을 확보한다.

#### 구현 단계

| Sub-Phase | 내용 | 변경 범위 |
|-----------|------|-----------|
| **A** | Backend 분리 + 테스트 | `analyzer.py` → `detect_only()` + `convert_regions()` 추출, `main.py` 커맨드 추가, `test_analyzer_split.py` |
| **B** | IPC 레이어 | `main.ts` + `preload.ts` 핸들러, `types/index.ts` 타입 추가 |
| **B-1** | 프론트엔드 테스트 러너 | `vitest` + `@testing-library/react` 설정, `package.json` test 스크립트 |
| **C** | OdReviewOverlay 컴포넌트 + 테스트 | 신규 `OdReviewOverlay.tsx`, 상태 전이 테스트 |
| **D** | AreaCapture 통합 | PendingOdReview 세션, navigation lock, reading-order 재정렬, 재시도 보존 |
| **E** | 폴리시 | 단축키, 진행 표시, 이벤트 충돌 방지, 메모리 정리 |

#### 핵심 설계 결정
1. `capture:detect` (OD only) + `capture:convert` (Gemini + figure post-processing) IPC 분리
2. 기존 `capture:analyze` atomic 경로 하위 호환 유지
3. `PendingOdReview` 세션 객체로 detect 시점 source context 고정 (immutable)
4. convert 직전 reading-order 재정렬, 재시도 시 편집된 detections 보존
5. OD Review 토글 기본 OFF — 기존 사용자 영향 없음

#### 수용 기준
- [ ] OD Review OFF 시 기존과 동일한 동작
- [ ] OD Review ON 시 YOLO 검출 결과가 오버레이로 표시됨
- [ ] 바운딩 박스 이동/리사이즈/유형 변경/삭제/추가가 작동
- [ ] 편집 후 변환 시 figure 후처리(300DPI 렌더링 포함)가 정상 동작
- [ ] 리뷰 중 페이지 이동 시 discard 확인 다이얼로그 표시
- [ ] 변환 실패 후 재시도 시 편집된 detections 보존
- [ ] `test_analyzer_split.py` 전체 통과
- [ ] 프론트엔드 상태 전이 테스트 전체 통과

> 상세 계획서: `doc/plans/v2.1_OD_Review_Step.md`
...
| 2026-03-24 | v2.3 | v2.1 OD Review Step 추가: Phase 12 정의, §4.3.5 OD Review Step 섹션 신설, 로드맵 v2.1 추가, §13.8 상세 구현 계획 추가. Codex 교차검토 Finding 1~4 반영(figure 후처리, PendingOdReview 세션, reading-order 재정렬, 테스트 인프라) |
